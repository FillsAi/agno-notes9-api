...
async def chat_response_streamer(team: Team, message: str) -> AsyncGenerator:
    ...
    """
    run_response = team.arun(message, stream=True)
...
async def run_team(team_id: TeamType, body: RunRequest):
    ...
    if body.stream:
        ...
        # Use synchronous run for better reliability
        # Force synchronous operation to avoid any async issues
        response = team.run(body.message, stream=False)
